...
var FinishedHouses = scope.getBuildings({type: "House", player: me, onlyFinished: true});
...
if(Castles.length >= 1){
    for(let l = 0; l < Castles.length; l++){
        ...
        if(saving == 0){ //Is AI saving?
            ...
            // Only build first house if no finished houses exist yet
            ...
            // Build barracks right after first house is finished
            if(FinishedHouses.length > 0 && Rax.length === 0){
                if(gold >= 125){
                    if(spotfound3 === 1){
                        scope.order("Build Barracks", miningWorkers, {x: buildlocation3x3x, y: buildlocation3x3y});
                    }
                }
            }else if(Rax.length > 0){
                // After barracks is built, build more houses and guilds as needed
                if(gold >= 125){
...
if(time >= 240){
    if(enemyUnits.length > 0){
        ...
        var allEnemyPlayers = scope.getArrayOfPlayerNumbers();
        for(var ep = 0; ep < allEnemyPlayers.length; ep++){
            if(scope.getTeamNumber(allEnemyPlayers[ep]) !== myTeam){
                var startLoc = scope.getStartLocationForPlayerNumber(allEnemyPlayers[ep]);
                scope.order("AMove", units, {x: startLoc.x, y: startLoc.y});
                break;
            }
        }
    }
